...
// return hex representation of digest as 4 blocks of 8 bytes
uint32_t *MD5::result() const {
  if (!finalized)
    return {};

  uint32_t *blocks = new uint32_t[4];
  for (int i = 0; i < 16; i += 4)
    blocks[i / 4] = (uint32_t)digest[i] << 24 | (uint32_t)digest[i + 1] << 16 | (uint32_t)digest[i + 2] << 8 |
                    (uint32_t)digest[i + 3];
...
uint32_t *md5(const std::string str) {
  MD5 md5 = MD5(str);
  return md5.result();
}